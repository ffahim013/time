      ...
      <div className="max-w-7xl mx-auto glass-panel px-8 py-4 rounded-[32px] flex justify-between items-center">
        <div className="flex items-center gap-4">
          ...
          <div className="hidden md:block">
            <div className="text-sm font-bold tracking-tight leading-none uppercase text-white">Chronos<span className="text-cyan-500">.</span></div>
            <div className="mono text-[8px] opacity-40 uppercase tracking-[0.4em] mt-1 text-white">অ্যাডমিন কন্ট্রোল v2.0</div>
          </div>
        </div>
          ...
          {user ? (
            <div className="flex items-center gap-6">
              <div className="text-right">
                ...
                <div className="mono text-[9px] opacity-40 uppercase text-white">{user.role === 'admin' ? 'অ্যাডমিন' : 'ভিজিটর'}</div>
              </div>
              {user.role === 'admin' && (
                <button 
              ...
              <button onClick={onLogout} className="opacity-30 hover:opacity-100 transition-opacity text-white">
            ...
            <button 
              ...
              className="mono text-xs font-bold tracking-widest uppercase opacity-60 hover:opacity-100 transition-opacity text-white"
            >
              সিস্টেম_লগইন
            </button>
          )}
